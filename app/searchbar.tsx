...
const API_URL = process.env.PUBLIC_API_URL || "http://127.0.0.1:3031";
...
const SearchBar = ({ setDataValues }: { setDataValues: Dispatch<any> }) => {
  ...
  const onJwtChange = (e: SyntheticEvent<HTMLInputElement>) =>
    setJwt(e.currentTarget.value);
  const onOpenChange = (e: boolean) => setModalOpen(e);
  ...
  return (
    <>
      <form
        className="flex flex-col md:flex-row py-2 px-2 gap-2 place-content-between border-b"
        onSubmit={onAPIEvent}
        noValidate
      >
        <div className="flex gap-3 items-center">
          ...
          <div>
            <h1 className="text-xl">Spider Crawler Live Preview</h1>
            <p className="text-sm">
              Example app for{" "}
              <a
      ...
      {modalOpen && (
        <Dialog open={modalOpen} onOpenChange={onOpenChange}>
          ...
          <DialogContent className="p-4 bg-white rounded-md shadow-md">
            <DialogHeader>
              <DialogTitle>Set Spider API Key</DialogTitle>
              <DialogDescription>
                Set your Spider API key to get started. The token will not be
                used outside of your local computer and is not stored locally.
                Make sure to write it down somewhere to re-use it.
              </DialogDescription>
            </DialogHeader>

            <div className=" flex gap-3">
              <label htmlFor="jwt" className="sr-only">
                Enter Spider API Token:
              </label>
              <Input
                type="text"
                id="jwt"
                className="p-2 flex-1 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={jwt}
                onChange={onJwtChange}
                placeholder="sk-134fef42..."
              />
              <Button
                type="button"
                onClick={closeModal}
